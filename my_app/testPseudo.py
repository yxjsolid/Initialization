...
class MyCanvas(wx.ScrolledWindow):
    ...
    def openImage(self):
        #name1 = r"D:\workspace\myGitProj\init\my_app\image\btn1.bmp"
        name1 = r"D:\workspace\myGitProj\init\my_app\image\btn1.png"
        name2 = r"D:\workspace\myGitProj\init\my_app\image\btn2.png"


        #self.btn1 = Image.open(r"D:\workspace\myGitProj\init\my_app\image\btn1.bmp")
        #self.btn2 = Image.open(r"D:\workspace\myGitProj\init\my_app\image\btn2.png")
        ...
        self.btn1 = img1.ConvertToBitmap()
        self.btn2 = img2.ConvertToBitmap()

        #self.btn1.show()
        #print self.btn2

#
        #bitmap = wx.BitmapFromImage(self.btn1, depth=-1)

        #self.bmp1 = self.btn1.tobitmap()


        #pilImage = Image.open(r"D:\workspace\myGitProj\init\my_app\image\btn1.png")
        #image = wx.EmptyImage(pilImage.size[0],pilImage.size[1])
        #image.putdata(pil.convert("RGB").tostring())
        #image.setAlphaData(pil.convert("RGBA").tostring()[3::4])

## use the wx.Image or convert it to wx.Bitmap
       # bitmap = wx.BitmapFromImage(image)

    def DoMyDrawBmp(self):
      

        self.on = 0
        id = wx.NewId()
        self.pdc.SetId(id)
        w,h = self.btn1.GetSize()
        x = random.randint(0, W-w)
        y = random.randint(0, H-h)
        self.pdc.DrawBitmap(self.btn1,x,y,True)
        self.pdc.SetIdBounds(id,wx.Rect(x,y,w,h))
        self.objids.append(id)


    def DoMyBmpSwitch(self, id):
      
        ro = self.pdc.GetIdBounds(id)
        x = ro[0]
        y = ro[1]
        w,h = self.btn1.GetSize()
       ...
       #self.pdc.SetIdBounds(id,r)
        ...
        if self.on:
            ...
            self.on = 1
            self.pdc.DrawBitmap(self.btn2,x,y,True)
    ...
    def DoMyDraw(self,id):
        ...
        pen = self.RandomPen()
        self.pdc.SetPen(pen)
        self.pdc.SetBrush(self.RandomBrush())
        r = wx.Rect(x-l,y-l,2*l,2*l)
...
class MyApp(wx.App):
    ...
    def OnInit(self):
        ...
        self.frame.Show()
        runTest(self.frame, self.frame, None)
